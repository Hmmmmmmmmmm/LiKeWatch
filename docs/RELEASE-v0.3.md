...
Camera/screen permission attribution and actual Telegram delivery require hardware/account testing. Cleanup previews eligible artifacts; `cleanup --apply` removes only authenticated, unchanged artifacts that are no longer referenced. Repair can rebuild damaged environments from authenticated packages. A maintenance-runtime upgrade or missing recovery inputs require a new installer. Intel Mac builds are not included.
