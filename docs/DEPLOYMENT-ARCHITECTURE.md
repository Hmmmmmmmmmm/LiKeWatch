...
Activation requires the managed app to stop cooperatively and return a nonce-bound restart request. The supervisor starts a paused trial and checks its PID, session, nonce, source, environment, and transaction before atomically replacing the active record. The previous tuple remains available. Ordinary app exit does not restart or roll back. An owned stdin pipe binds GUI lifetime to its supervisor; supervisor death closes the GUI cooperatively instead of leaving an orphan holding the data lock. Interrupted pre-commit transactions leave the previous deployment selected; an atomic committed record always contains a complete source/environment pair.
