...
def validate(profile):
    if profile.schema != 1 or profile.source not in ("demo", "camera", "screen", "image"):
    ...
    if not all(isinstance(x, (int, float)) and math.isfinite(x) for x in (profile.interval, profile.freshness)):
        raise ValueError("Timing must be finite")
    if not .2 <= profile.interval <= 3600 or not profile.interval <= profile.freshness <= 86400:
        raise ValueError("Freshness must be at least the capture interval (0.2–3600 seconds)")
    if len(profile.regions) > 32 or len(profile.rules) > 64:
        raise ValueError("Maximum 32 regions and 64 rules")
    if len({r.id for r in profile.regions}) != len(profile.regions) or len({r.id for r in profile.rules}) != len(profile.rules):
        raise ValueError("Duplicate region/rule IDs")
    for region in profile.regions:
        validate_corners(region.corners)
        if region.source_size and (len(region.source_size) != 2 or any(not isinstance(x, int) or x < 5 for x in region.source_size)):
            raise ValueError("Invalid source dimensions")
        if not region.name.strip() or len(region.name) > 100 or region.kind not in ("number", "text"):
            raise ValueError("Invalid variable name or type")
        if region.preprocessing not in ("gray", "invert", "otsu", "adaptive") or region.psm not in (7, 8):
        ...
        if region.minimum and region.maximum and Decimal(region.minimum) > Decimal(region.maximum):
            raise ValueError("Minimum exceeds maximum")
    for rule in profile.rules:
        if not rule.name.strip() or not 1 <= rule.confirm <= 100 or not 1 <= rule.recover_confirm <= 100:
    ...
    if not isinstance(profile.data_interval, int) or not 0 <= profile.data_interval <= 86400:
        raise ValueError("Data interval must be 0 (disabled) through 86400 seconds")
    if any(route not in ("ALERT", "RECOVERY", "DATA", "SYSTEM", "TEST") for route in profile.routes):
        raise ValueError("Unknown message route")
    if profile.topic_id and (not profile.topic_id.isdigit() or int(profile.topic_id) <= 0):
...
def demo_profile():
    a = Region("Temperature", [[.06,.29],[.35,.29],[.35,.44],[.06,.44]], unit="°C")
    b = Region("Pressure", [[.56,.29],[.85,.29],[.85,.44],[.56,.44]], unit="kPa")
    for r in (a,b):
        r.source_key = 'demo:0:'
        r.source_size = [1000,650]
    condition = {"group":"ALL", "children":[{"variable":a.id,"op":">","value":"80"}, {"variable":b.id,"op":"<","value":"20"}]}
    recovery = {"group":"ANY", "children":[{"variable":a.id,"op":"<","value":"78"}, {"variable":b.id,"op":">","value":"22"}]}
    return Profile(regions=[a,b], rules=[Rule("High temperature / low pressure", condition, recovery=recovery)])
