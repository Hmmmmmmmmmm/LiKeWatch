...
class Store:
    ...
    def enqueue(
        self, profile, kind, body, incident=None, rule=None, event_id=None, now=None
    ):
        ...
        with self.connection() as db:
            ...
            if kind == "ALERT" and rule:
                db.execute(
                    "INSERT INTO incidents(id,profile,rule_id,name,activated) VALUES(?,?,?,?,?)",
            ...
            if profile.delivery_enabled and profile.chat_id and kind in profile.routes:
                ...
                if len(encoded) > 7600:
                    body = (
                        encoded[:7400].decode("utf-16-le", errors="ignore")
                        + "\n[Truncated; complete event stored locally]"
                    )
                db.execute(
                    "INSERT INTO outbox(event_id,profile,incident,kind,chat,topic,body,state,due,created) VALUES(?,?,?,?,?,?,?,?,?,?)",
                    (
                        event_id,
                        profile.id,
                        incident,
    ...
    def delivered(self, seq, state, delay=0, detail=""):
        with self.connection() as db:
            db.execute(
                "UPDATE outbox SET state=?,due=?,detail=? WHERE seq=?",
                (state, time.time() + delay, detail, seq),
            )
