...
def run():
    ...
    profile=demo_profile(); engine=OcrSupervisor(timeout=40)
    try:
        observations,previews=engine.analyze(demo_frame(),profile.regions,time.time(),1)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            assert len(window.observations)==2, 'GUI pipeline did not publish observations'
            ...
            window.worker.stopping.set(); window.delivery.stopping.set()
            assert window.worker.wait(5000); assert window.delivery.wait(15000)
            window.close(); app.processEvents()
        result={'status':'passed','engine':'Tesseract English','values':[str(v) for v in values],'gui':'passed'}
        if '--report' in __import__('sys').argv:
            index=__import__('sys').argv.index('--report')
            Path(__import__('sys').argv[index+1]).write_text(json.dumps(result),encoding='utf-8')
