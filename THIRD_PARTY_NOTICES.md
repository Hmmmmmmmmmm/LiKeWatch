...
includes package metadata and licenses retained in the private environments, bundled wheels, and conda package cache (or retained by PyInstaller for legacy builds). Qt is dynamically
...
The managed distribution additionally includes conda (BSD-3-Clause), Git
(GPL-2.0), cryptography (Apache-2.0/BSD), packaging (Apache-2.0/BSD), and
constructor-generated installation support (BSD-3-Clause). Git is invoked as a
separate maintenance process. Exact conda versions, builds, download URLs, and
hashes are recorded in `deployment/locks/*-maintenance.json`. Package archives
and their license/recipe metadata are retained in the installation's package
cache. Git upstream source is available at https://git-scm.com/downloads/source;
conda-forge build recipes and patches are at
https://github.com/conda-forge/git-feedstock. These component licenses remain
applicable independently of the LiKeWatch application source.
